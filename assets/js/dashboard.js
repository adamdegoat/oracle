...
function gameBadge(game) {
  ...
  if (g.includes('cs') || g === 'csgo') return `<span class="badge badge-home">CS2</span>`;
  if (g.includes('lol'))                return `<span class="badge badge-away">LoL</span>`;
  return `<span class="badge badge-draw">${game || '?'}</span>`;
}

function selBadge(sel) {
  const map = {
    TEAM_A: 'home', TEAM_B: 'away',
    FAVORITE: 'win', UNDERDOG: 'loss',
  };
  const cls = map[sel] || 'draw';
...
async function loadEquityCurve() {
  ...
  if (!d || !d.curve || d.curve.length < 2) {
    document.getElementById('equity-chart').parentElement.innerHTML =
      '<div class="empty">Equity curve appears after first settled bet</div>';
    return;
  }

  const labels = d.curve.map(p => p.date || 'Start');
  const values = d.curve.map(p => p.equity);
...
async function loadOpenBets() {
  ...
  tbody.innerHTML = d.bets.slice(0,20).map(b => `
    <tr>
      <td>${b.team_a || b.home_team || '?'} vs ${b.team_b || b.away_team || '?'}</td>
      <td>${gameBadge(b.game)}</td>
...
async function loadFixtures() {
  ...
  if (!d || !d.fixtures || d.fixtures.length === 0) {
    tbody.innerHTML = '<tr><td colspan="4" class="empty">No upcoming matches fetched — API key needed or off-season</td></tr>';
    return;
  }
  tbody.innerHTML = d.fixtures.slice(0,20).map(f => {
    const firing = (f.active_strategies_firing || []);
    const signal = firing.length
      ? `<span style="color:var(--accent);font-size:10px">▲ ${firing.join(', ')}</span>`
      : '<span style="color:var(--muted);font-size:11px">—</span>';
    return `
    <tr>
      <td>${f.team_a || ''} vs ${f.team_b || ''}<br>
        <span style="font-size:10px;color:var(--muted)">${f.tournament || ''}</span></td>
      <td>${gameBadge(f.game)}</td>
...
async function loadRecentBets() {
  ...
  if (!d || !d.bets || d.bets.length === 0) {
    tbody.innerHTML = '<tr><td colspan="7" class="empty">No settled bets yet</td></tr>';
    return;
  }
...
async function loadCatalog() {
  ...
  if (!d || !d.strategies || d.strategies.length === 0) {
    tbody.innerHTML = '<tr><td colspan="7" class="empty">L9 inventing strategies — check back soon</td></tr>';
    return;
  }
  tbody.innerHTML = d.strategies.map(s => {
    ...
    <tr>
      ...
      <td style="font-size:11px;color:var(--muted);max-width:180px">${s.rationale||'—'}</td>
    </tr>`;
  }).join('');
}
